...
(function(win,lkMusic,WaveSurfer){ 
  ...
  LMusic.prototype ={
    config:{
      ...
      showrightmenu:true,
      gradientleft:'#fff',
      gradientmiddle:'#FFFFB3',
      gradientright:'#FFB3B3',
      gradienttop:'#FFB3B3',
      canvascontainer:".music-container-spectrum",
      spectrumtype:0,
      showlist:true,//是否显示歌词
      callback:function(obj){
      	
      },
      callbackinit:function(obj){
    ...
    prepareAudioApi:function(){
          ...
          window.cancelRequestAnimationFrame =  window.cancelAnimationFrame || window.mozCancelAnimationFrame ||window.webkitCancelAnimationFrame || window.msCancelAnimationFrame;
    ...
    drawSpectrum1: function(analyser) {
    	var that = this,
            canvas = document.querySelector(this.config.canvascontainer),
            cwidth = canvas.width,
    ...
    drawSpectrum: function(analyser) {
        var that = this,
            ...
            capHeight = 2,
            capStyle = this.config.gradienttop,
            meterNum = 600 / (meterWidth + gap), //count of the meters
            capYPositionArray = []; ////store the vertical position of hte caps for the preivous frame
        ctx = canvas.getContext('2d'),
    ...
    hideProgress:function(target){
         ...
         //开始进行音频频谱的绘制
         target.switchSpectrumMode(target.config.spectrumtype);
    },
    switchSpectrumMode:function(mode){
    	if(!mode){
    		mode = 0;
    	}
    	switch(mode){
    		 ...
    		 this.drawSpectrum(LkMusic.WebAudio.analyser);
    		 break;
    		 case 1:
    		 cancelRequestAnimationFrame(this.animationId);
    		 this.animationId = undefined;
    		 this.drawSpectrum1(LkMusic.WebAudio.analyser);
    		 break;
    	}
    },
    ...
    action:function(){
		...
		if(_this.config.autoPlay){
          ...
          typeof this.config.callbackinit === 'function' && this.config.callbackinit(_callback);
...
//              lyricWrap : $('.grid-music-container .js-music-lyric-content'), //歌词区域
                listWrap : $('.m-music-list-wrap'),
                lyricWrap : $('.js-music-lyric-content'), //歌词区域
                volume   : {
    ...
    playprev:function(){
    	this.playByMode('prev');
    },
    playended:function(){
    	this.playByMode('ended');
    }
  };
  //这里是关键代码用来拓展观察者模式来着
